...
**Workflow chain:** `/write-a-prd` → **`/prd-to-plan`** (you are here) → `/plan-to-issues` → `/ralph` → `/review-to-issues` → `/ralph` (cycle)
...
After writing the plan file, suggest the next step in the chain: "Run `/plan-to-issues` to create GitHub issues from these phases."
